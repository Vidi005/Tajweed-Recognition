...
const PdfSettingPrompt = ({ props, isPromptOpened, isOCREnabled, enableOCRMode, confirmSetting, cancelRecognition }) => (
  <Transition appear show={isPromptOpened} as={Fragment}>
    <Dialog className={"pdf-setting-modal relative z-20"} onClose={cancelRecognition}>
      <Transition.Child
        as={Fragment}
      ...
      >
        <Dialog.Panel className={"flex flex-col items-center min-h-full max-w-5xl mx-4 bg-green-800 dark:bg-gray-700 text-green-900 dark:text-white shadow-lg dark:shadow-white/50 rounded-lg overflow-hidden"}>
          <section className="inline-flex w-full justify-between p-4 text-center dark:text-white">
            <h3 className="grow text-green-50">{props.t('additional_setting')}</h3>
          ...
          <section className="flex flex-col items-center p-4 bg-green-50 dark:bg-gray-900 text-base lg:text-lg">
            <p className="text-justify text-green-900 dark:text-white leading-tight">{props.t('additional_setting_desc')}</p>
            <Listbox value={isOCREnabled} onChange={enableOCRMode} className="w-full p-1 bg-green-50 dark:bg-gray-900 text-green-900 dark:text-white">
              <Listbox.Options static className="max-h-full flex flex-col items-center justify-center content-center">
                <Listbox.Label className={"cursor-pointer px-2 py-3"}>
                  ...
                  />
                  <span className="ml-2 text-base md:text-lg">{props.t('enable_ocr')}</span>
                </Listbox.Label>
                <button className="w-fit m-2 px-8 py-2 bg-green-800 hover:bg-green-900 active:bg-green-500 text-center text-white rounded-md shadow-md dark:shadow-white/50 duration-200" onClick={confirmSetting}>{props.t('ok')}</button>
              </Listbox.Options>
            </Listbox>
          </section>
        </Dialog.Panel>
